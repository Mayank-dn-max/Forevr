...
======================================
Trace AI adapter for LangChain / LangGraph.

How it works:
    LangChain fires callbacks (on_llm_start, on_tool_start, etc.) at every
    step of execution. This handler translates those events into Trace AI spans.

Usage (zero config — one line):
    from integrations.langchain_integration import TraceAICallbackHandler

    handler = TraceAICallbackHandler()
...
What gets traced automatically:
    - Every chain invocation  → phase span
    - Every LLM call          → llm_call span with token counts
    - Every tool call         → tool_call span with input/output
    - Errors at any level     → failed span with error message
    - Final agent response    → agent_response span
"""
...
class TraceAICallbackHandler(BaseCallbackHandler):
    ...
    def __init__(self, agent_name: str = "langchain_agent"):
        ...
        # Maps LangChain run_id → our span_id (for proper parent linking)
        self._run_to_span = {}
    ...
    def on_chain_start(self, serialized, inputs, *, run_id, parent_run_id=None, **kwargs):
        if not self._trace_open:
            # Outermost chain = start of a new agent run
